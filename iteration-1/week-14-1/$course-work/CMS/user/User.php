...
//        User::$isLoged  = true;
    ...
    public static function login($email, $password) {
        ...
        if(User::isAvailable($email, $password)) {
            ...
            \db\Database::getInstance()->query("SELECT * FROM cms.users WHERE email = '{$email}' AND password = '{$password}'");
            $userObject             =  \db\Database::getInstance()->fetch();            

            $userRole               = $userObject['role'];
            \db\Database::getInstance()->query("SELECT resource FROM cms.cms_http_resources WHERE role = {$userRole}");
            $perimitionCollection   = \db\Database::getInstance()->fetchCollection();
            $userObject['permitions'] = $perimitionCollection;

...
class Database {
    ...
    static function expression($columnLeft, $sign, $columnRight) {
        return "$columnLeft $sign $columnRight";
    }
    ...
    static function query($sqlQuery) {
        ...
        if(!$result) {
//            TODO: remove from global buffer          
    ...
    static function hasError() {
        ...
        $databaseConnection = self::getConnection();
        $dbError            = mysqli_error($databaseConnection);
        $hasError           = strlen($dbError) > 0;
        return $hasError;
    }
    ...
    static function where($queryPropertyCollection) {
        // WHERE a = ? AND / OR b = ?
        if(count($queryPropertyCollection) == 0) {                
            return __CLASS__;
        }
        ...
        foreach ($queryPropertyCollection as $key => $value) {
            ...
            if($key == '@expresion') {
                array_push($whereQueryArray, ("$value"));
            }
            else {
                array_push($whereQueryArray, ("$key LIKE $value"));
            }
        }
    ...
    static function limit($limitQuery) {
        // WHERE a = ? AND / OR b = ?
        if(count($limitQuery) == 0) {                
            return __CLASS__;
        }
        
        $limit = $limitQuery['limit'];
        $offset = $limitQuery['offset'];
